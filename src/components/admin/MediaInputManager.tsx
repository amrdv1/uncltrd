  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <label className="block text-sm font-bold text-zinc-800 dark:text-zinc-200 uppercase">Додаткові Медіа (Галерея)</label>
        <button type="button" onClick={addMedia} className="text-xs bg-zinc-200 hover:bg-zinc-300 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-black dark:text-white px-3 py-1 rounded font-bold uppercase flex items-center">
          + Додати
        </button>
      </div>
      ...
      {media.map((item, index) => (
        <div key={index} className="flex space-x-2 items-center bg-zinc-50 dark:bg-zinc-900 p-2 border border-zinc-200 dark:border-zinc-800 rounded">
          <select 
            value={item.type} 
            onChange={(e) => updateMedia(index, "type", e.target.value)}
            className="px-3 py-2 border border-zinc-300 dark:border-zinc-700 rounded text-sm outline-none bg-white dark:bg-black text-black dark:text-white font-bold uppercase w-1/4"
          >
          ...
          <div className="flex-1 flex space-x-2">
            <input 
              ...
              className="px-3 py-2 border border-zinc-300 dark:border-zinc-700 rounded text-sm outline-none flex-1 bg-white dark:bg-black text-black dark:text-white"
            />
